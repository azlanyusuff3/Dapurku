...
function openItemForm(id=null,prefillBarcode=''){
  Promise.resolve(id?getOne('items',id):null).then(item=>{item=item||{name:'',category:'Barang Kering',status:'in',favorite:false,expiry:'',barcode:prefillBarcode,qty:1,unit:'',icon:''};
    ...
    $('#itemForm').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.target);const val={...item,id:id||uid(),name:f.get('name').trim(),icon:f.get('icon')||autoItemIcon(f.get('name')),category:f.get('category'),status:f.get('status'),qty:Number(f.get('qty')||0),unit:f.get('unit').trim(),expiry:f.get('expiry'),barcode:f.get('barcode').trim(),favorite:f.get('favorite')==='on',lastPurchasedAt:f.get('purchaseDate')||item.lastPurchasedAt||'',createdAt:item.createdAt||todayISO(),updatedAt:todayISO()};delete val.location;await put('items',val);await syncAutoShopping();closeModal();toast(id?'Pantry item updated':'Pantry item added');render()};
